(function ($){
    $(document).ready( function () {
        ...
        $(document).on( 'change', '.br_grid_list_style_select, .br_grid_list_device_select', function() {
            var list_style = $('.br_grid_list_style_select').val();
            if( list_style == 'disable' ) {
                $('.br_grid_list_device_select').val('pages');
            }
        });
        $(document).on('click', '.br_grid_list_page_add', function(event) {
            ...
            if( $('.brlgv_already_exist').length > 0 ) {
                $('.brlgv_already_exist').removeClass('brlgv_already_exist');
            }
            if ( ( device == 'pages' && $('.id_'+page_id).length == 0 ) 
              || ( device != 'pages' && $('.id_pages_'+page_id).length == 0 && $('.id_'+device+'_'+page_id).length == 0 )
              || ( list_style == 'disable' && $('.id_pages_'+page_id).length == 0 && $('.id_mobile_pages_'+page_id).length == 0 && $('.id_desktop_pages_'+page_id).length == 0 )
            ) {
                if( list_style == 'disable' ) {
                    device = 'pages';
                }
                ...
                if( list_style == 'disable' ) {
                    if( $('.id_'+page_id).length > 0 ) {
                        $('.id_'+page_id).addClass('brlgv_already_exist');
                    }
                } else {
                    if( device == 'pages' ) {
                        if( $('.id_'+page_id).length > 0 ) {
                        ...
                        if( $('.id_pages_'+page_id).length > 0 ) {
                            $('.id_pages_'+page_id).addClass('brlgv_already_exist');
                        }
                        if( $('.id_'+device+'_'+page_id).length > 0 ) {
                            $('.id_'+device+'_'+page_id).addClass('brlgv_already_exist');
                        }
                    }
                }
            }
